...
//Flags for SubBand cycling
int currentSubBand = 1;               // Start with SubBand 1
unsigned long subBandStartTime = 0;   // Timing of when we start the cycle
const unsigned long subBandDuration = 30000; // 30 seconds between switches
bool subBandJoining = false;          // True once join attempted
...
// Call this when Transmission sucessful to reset ACKSENT
void resetACK() {
    espReceived = false;
}
    ...
    switch(ev) {
        ...
        case EV_TXCOMPLETE:
            ...
            if (LMIC.txrxFlags & TXRX_NACK) {
              ...
              if (msgSent) { //Only when message is sent will ACK be sent
                TX_success =true; //Successful transmission
              }
            }
            break;
        case EV_TXSTART:
...
//SubBandCycle will continuously cycle through each SubBand until a session is joined
void SubBandCycle() {
  if (join) return; // Stop switching if already joined
  
  unsigned long start = millis();
   // Wait 1 minute before trying next subband
  if (subBandJoining && start - subBandStartTime >= subBandDuration && !join) {
    currentSubBand = (currentSubBand + 1) % 3; // Will cycle through each subBand continuously
    Serial.print("Switching to SubBand ");
    Serial.println(currentSubBand);
    LMIC_selectSubBand(currentSubBand);
    LMIC_reset();
    LMIC_startJoining();
    subBandStartTime = start; // Reset timer
  }
}
  ...
  // Start with SubBand 1
  subBandStartTime = millis();
  currentSubBand = 1;
  LMIC_selectSubBand(currentSubBand);
  ...
  // Try to restore session before initiating join
  ...
  if (!sessionRestored && first_join) {
    ...
    subBandJoining = true; // Set your flag to allow subband cycling
    subBandStartTime = millis();
    first_join = false;
  } else {
...
void loop() {
  os_runloop_once();
  SubBandCycle(); //Start systematically going through the different subBands endlessly until successfully joined
  if (Serial1.available() && join) { //change to serial if running tests from computer with Board alone or serial1 for integrated circuit
    String input = Serial1.readStringUntil('\n'); //change to serial if running tests from computer with Board alone or serial1 for integrated circuit
    input.trim();

    Serial.print("Serial Monitor Input: ");
    Serial.println(input);

    if ((input.length() < MAX_LENGTH) && (input.startsWith("coyote") || input.startsWith("bobcat") || input.startsWith("cougar"))) {
