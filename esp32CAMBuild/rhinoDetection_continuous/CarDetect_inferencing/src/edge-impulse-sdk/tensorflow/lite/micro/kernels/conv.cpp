...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    data_dims_t input_dims =  {
                                ...
                                .channels = input->dims->data[3]// Original .channels = input->dims->data[3], 1 
                              };
    data_dims_t output_dims = {
                                .width = output_width, .height = output_height,
                                .channels = output->dims->data[3]//Original .channels = output->dims->data[3]
                              };
    data_dims_t filter_dims = {.width = filter_width, .height = filter_height}; //Original.width = filter_width, .height = filter_height, 0, 0
...
inline void EvalQuantizedPerChannel(
  ...
  if (dilation_width_factor == 1 && dilation_height_factor == 1) {
    ...
    data_dims_t input_dims =  {
                                .width = input_width, .height = input_height,
                                .channels = input_depth //Original .channels = input_depth, 1
                              };
    data_dims_t output_dims = {
                                .width = output_width, .height = output_height,
                                .channels = output_depth //Original .channels = output_depth, 1
                              };
    data_dims_t filter_dims = {.width = filter_width, .height = filter_height}; //Original .width = filter_width, .height = filter_height, 0, 0
    conv_params_t conv_params = {
